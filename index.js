...
//import monthly foreign tourists data from country_values
  ...
  var svgMargin = {left: 20, top: 40, right: 20, bottom: 0}
    ...
    width = Math.min(screenWidth, 750) - (svgMargin.left) - svgMargin.right,
    height = 1100;
  ...
  var barMargin = {top: 20, right: 30, bottom: 65, left: 40}, //margins in D3 are specified as an object with top, right, bottom and left properties
      ...
      barHeight = Math.min(screenHeight, 250) - barMargin.top - barMargin.bottom;

  var barMargin2 = {top: 20, right: 30, bottom: 65, left: 40},
  barWidth2 = Math.min(screenWidth, 350) - barMargin2.left - barMargin2.right,
...
// now add titles to the axes
  chart2.append("text")
    .attr("text-anchor", "middle")  
    .attr("transform", "translate("+ (barMargin2.bottom)+")rotate(0)")  
    .text("Number of Guests");
  chart2.append("text")
    .attr("text-anchor", "middle")  // this makes it easy to centre the text as the transform is applied to the anchor
    .attr("transform", "translate("+ (barWidth2/2) +","+(barHeight2-(barMargin2.bottom - 100))+")")  // centre below axis
...
// this also translates each g element - https://github.com/d3/d3-selection#selection_enter
  ...
  var barChart3 =	chart3.selectAll(".bar")
    ...
    // Sets the actual position of the legend  container
      .attr("transform", "translate(" + (width/2.2) + "," + 800 + ")");
    //Draw the Rectangle
